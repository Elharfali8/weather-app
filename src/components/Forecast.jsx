import { sun, snow, raining, cloud, cloudy, moon } from '../assets';
...
const Forecast = ({ data }) => {
    ...
    const isNightTime = () => {
        ...
        return currentHour >= 18 || currentHour < 6; // Night is between 6 PM and 6 AM
    };
    ...
    const getWeatherImage = (main) => {
        switch (main) {
            case 'Clear':
                return isNightTime() ? (
                    <img src={moon} alt="moon" className='w-full max-w-[148px]' />
                ) : (
        ...
        >
            <div className='grid gap-y-3 main-opacity py-2 px-4 rounded-xl'>
                <div className='mb-4 lg:mb-8'>
                    <h1 className='poppins-semibold tracking-wide text-2xl sm:text-3xl md:text-4xl lg:text-5xl'>
                        {name}
                    </h1>
                    <p className='text md:text-lg'>{weather[0].description}</p>
                </div>
                <h1 className='text-3xl sm:text-4xl md:text-5xl lg:text-6xl tracking-wide poppins-bold'>
                    {main.temp}&deg;
                </h1>
            </div>
